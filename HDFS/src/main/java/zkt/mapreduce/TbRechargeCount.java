...
public class TbRechargeCount extends Configured {
	...
	public static class IntSumReducer extends Reducer<Text, Text, Text, Text> {
		...
		public void reduce(Text key, Iterable<Text> values, Context context) throws IOException, InterruptedException {
			...
			String sql = "insert into t_user_czxf(time,num,type,category) values ('" + currentDate + "','" + f.toString()
					+ "','0','0')";
			try {
			...
			// mos.write("tbDayCount", key, result);
		}
	}
	...
	public static void main(String[] args) throws Exception {
		...
		/*
		 * FileSystem fs = FileSystem.get(conf); Path outPath = new Path(args[1]); // 如果输出路径已存在，则先删除 if (fs.exists(outPath)) {
		 * fs.delete(outPath, true); System.out.println(outPath + "输出路径存在，已删除！"); fs.close(); }
		 */
		Job job = Job.getInstance(conf, "TbRechargeCount");
		job.setJarByClass(TbRechargeCount.class);
		...
		MultipleOutputs.addNamedOutput(job, "TbRechargeCount", TextOutputFormat.class, Text.class, Text.class);
